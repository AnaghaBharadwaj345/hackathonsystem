...
def add_history_route(patient_id: str, payload: HistoryCreate) -> Dict[str, Any]:
    if get_patient(patient_id) is None:
        raise HTTPException(status_code=404, detail="Patient not found")
    return add_history(patient_id, payload.condition, payload.notes)


@app.post("/api/patients/{patient_id}/admissions", status_code=201, tags=["patients"])
def add_admission_route(patient_id: str, payload: AdmissionCreate) -> Dict[str, Any]:
    if get_patient(patient_id) is None:
        raise HTTPException(status_code=404, detail="Patient not found")
    data = payload.model_dump(exclude_none=True)
    data["patient_id"] = patient_id
    try:
        return {"admission": add_admission(data)}
    except KeyError as exc:
        raise HTTPException(status_code=404, detail=f"Patient {exc.args[0]} not found") from exc


@app.patch("/api/admissions/{admission_id}/discharge", tags=["patients"])
def discharge_route(admission_id: str, payload: DischargeUpdate) -> Dict[str, Any]:
    try:
        return {"admission": discharge_admission(admission_id, payload.outcome, payload.notes)}
    except KeyError as exc:
        raise HTTPException(status_code=404, detail=f"Admission {exc.args[0]} not found") from exc
...
async def advance(run_id: str, payload: AdvanceIn) -> Dict[str, Any]:
    ...
    def work() -> Dict[str, Any]:
        with h.lock:
            ...
            return {"run_id": run_id, "minutes_advanced": ran, "snapshot": h.sim.snapshot()}
...
def surge(run_id: str, payload: SurgeIn) -> Dict[str, Any]:
    ...
    with h.lock:
        ...
        return {"run_id": run_id, "injected": payload.count, "snapshot": h.sim.snapshot()}
...
def reset(run_id: str) -> Dict[str, Any]:
    ...
    with h.lock:
        old = h.sim
        h.sim = engine.Sim(old.seed, old.policy, round(old.load * 100), old.capacity, old.horizon)
...
async def what_if(payload: WhatIfIn) -> Dict[str, Any]:
    """Price a staffing decision. Adds one unit of each resource in turn."""
...
async def live(
    ...
    """Streams a snapshot every interval, advancing `speed` simulated minutes each time."""
